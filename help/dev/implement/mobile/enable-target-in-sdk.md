...
1. 앱에 Adobe Mobile Services SDK을 설치하지 않았다면 Analytics 또는 Experience Cloud 자격 증명을 사용하고 [Adobe Mobile Services](https://mobilemarketing.adobe.com/) 웹 사이트에서 SDK을 다운로드하십시오.
...
1. 클라이언트 코드, 시간 제한을 추가하고, SSL을 활성화합니다.
   ...
   [!DNL Target] clientcode 및 시간 제한을 추가합니다. 클라이언트 코드는 계정 또는 회사에 고유합니다. 시간 제한은 [!DNL Target]이(가) 기본 콘텐츠를 표시하기 전에 응답을 기다리는 시간(초)입니다. Adobe Mobile Services의 앱 설정 관리 페이지에서 **[!UICONTROL HTTPS 사용]** 옵션이 선택되어 있는지 확인합니다. HTTPS를 사용하지 않는 경우 [!DNL Target] 서버를 사용하지 않으면 iOS+의 모든 호출이 차단됩니다.

   ![대체 이미지](assets/mobile-clientcode.png)

1. 앱을 만들었거나 찾은 후에 앱 설정을 찾고 원하는 SDK을 다운로드합니다.
