...
[!UICONTROL Adobe Experience Platform Web SDK] 및 기타 지원되는 SDK 통합에는 [!DNL Experience Platform Edge Network]을(를) 호출할 때 HTTP 요청 헤더에 브라우저와 유사한 `User-Agent` 값이 포함됩니다. SDK 없이 공개 [Edge API](https://developer.adobe.com/data-collection-apis/docs/getting-started/personalization){target=_blank}를 사용하는 서버측 통합은 이 헤더를 명시적으로 제공해야 합니다.

비 SDK Edge API 호출의 경우 다음 요구 사항을 준수하십시오.
