...
| enableRetries | 부울 | 아니요 | true | 소켓 시간 초과에 대한 자동 재시도(최대 4회) |
...
| defaultPropertyToken | 문자열 | 아니오 | 없음 | 모든 `getOffers` 호출에 대해 기본 속성 토큰을 설정합니다. **온디바이스 의사 결정**&#x200B;의 경우 SDK은 `defaultPropertyToken`에 설정된 속성 토큰에 대해 정규화된 활동이 포함된 아티팩트만 다운로드합니다. |
| defaultDecisioningMethod | DecisioningMethod enum | 아니요 | SERVER_SIDE | 온디바이스 의사 결정을 사용하려면 ON_DEVICE 또는 HYBRID로 설정해야 합니다. |
| 원격 분석 사용 | 부울 | 아니요 | true | 고객이 [!DNL Target] 서버에 요청하는 동안 추가 데이터 수집을 옵트아웃할 수 있도록 허용합니다. |
| proxyConfig | 클라이언트 프록시 구성 | 아니오 | 없음 | 클라이언트가 자체 프록시 세부 정보를 제공할 수 있음 |
| exceptionHandler | TargetExceptionHandler | 아니오 | 없음 | 규칙 처리 중 사용자 지정 예외 처리를 구현하는 데 사용할 수 있습니다. |
| httpClient | HttpClient | 아니오 | 없음 | 사용자가 [!DNL Target] HTTP 클라이언트를 사용자 지정 HTTP 클라이언트로 바꿀 수 있습니다. |
| onDeviceEnvironment | 문자열 | 아니요 | production | 스테이징과 같은 다른 온디바이스 환경을 지정하는 데 사용할 수 있습니다. |
...
| ondeviceArtifactPayload | 바이트[] | 아니오 | 없음 | 즉각적인 실행이 가능하도록 이전 아티팩트 페이로드와 함께 디바이스에서 의사 결정 제공 |
| onDeviceDecisioningHandler | OnDeviceDecisioningHandler | 아니오 | 없음 | 온디바이스 의사 결정 이벤트에 대한 콜백을 등록합니다. |
| onDeviceAllMatchingRulesMboxes | List\&lt;String\> | 아니오 | 없음 | 사용자가 온디바이스 의사 결정 중에 일치하는 모든 규칙 콘텐츠가 반환되는 mbox를 지정할 수 있습니다. |
