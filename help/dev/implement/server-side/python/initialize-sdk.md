...
| property_token | str | 아니오 | 없음 | [!DNL Target] 속성 토큰입니다. 여기에 지정하면 모든 get_offers 호출이 이 값을 사용합니다. |
...
| artifact_location | str | 아니오 | 없음 | [온디바이스 의사 결정 규칙 아티팩트](/help/dev/implement/server-side/sdk-guides/on-device-decisioning/rule-artifact-overview.md)에 대한 정규화된 URL입니다. 내부적으로 결정된 위치를 재정의합니다. |
| artifact_payload | 오브젝트 | 아니오 | 없음 | [온디바이스 의사 결정 규칙 아티팩트](/help/dev/implement/server-side/sdk-guides/on-device-decisioning/rule-artifact-overview.md)의 JSON 페이로드. 지정하면 URL에서 요청하는 대신 사용됩니다. |
| [events](sdk-events.md) | dict &lt;str, callable> | 아니오 | 없음 | 이벤트 이름 키와 콜백 함수 값이 있는 선택적 개체입니다 |
...
| version | str | 아니오 | 없음 | 이 SDK의 버전 번호 |
