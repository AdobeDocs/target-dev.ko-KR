...
그런 다음 SDK의 `Initialize` 메서드를 사용하여 SDK을 구성하고 초기화 단계를 완료합니다.

## 방법

`TargetClient`을(를) 사용하여 `TargetClient.Create`을(를) 만듭니다.
...
| OnDeviceDecisionReady | 액션 | 아니요 | null | 온디바이스 의사 결정 준비 이벤트에 대한 위임(온디바이스 의사 결정이 준비되면 한 번 호출됨) |
