...
description: '[!UICONTROL [!UICONTROL on-device decisioning]]을(를) 사용하여 서버에  [!DNL Target] A/B 및 MVT 활동을 캐시하여 거의 0에 가까운 대기 시간에 메모리 내 의사 결정을 수행하는 방법에 대해 알아봅니다.'
...
차세대 [!DNL Adobe Target] SDK는 이제 [!DNL Adobe Target]의 Edge Network에 대한 네트워크 요청을 차단하지 않고, 서버에서 A/B 및 Experience Targeting(XT) 캠페인을 캐시하고 거의 0에 가까운 대기 시간에 메모리 내 결정을 수행하는 기능을 제공하는 [!UICONTROL 온디바이스 의사 결정]을 제공합니다.

[!DNL Adobe Target]은(는) 또한 라이브 서버 호출을 통해 실험 및 ML 기반 개인화 캠페인에서 가장 관련성이 높고 최신 경험을 제공할 수 있는 유연성을 제공합니다. 즉, 성능이 가장 중요한 경우 [!UICONTROL 디바이스에서 의사 결정]을 활용하도록 선택할 수 있지만 가장 관련성이 높고 최신 경험이 필요한 경우에는 대신 서버 호출을 수행할 수 있습니다. 상호 사용을 보증하는 사용 사례에 대해 알아보려면 [장치 대 Edge Decisioning을 사용할 시기](../../sdk-guides/on-device-decisioning/supported-features.md)를 참조하십시오.

>[!NOTE]
>
>온디바이스 의사 결정은 클라이언트측과 서버측 구현 모두에서 사용할 수 있습니다. 이 문서에서는 서버측의 [!UICONTROL 디바이스에서 의사 결정]에 대해 설명합니다. 클라이언트측의 [!UICONTROL 온디바이스 의사 결정]에 대한 자세한 내용은 클라이언트측 구현 설명서 [여기](../../../client-side/atjs/on-device-decisioning/on-device-decisioning.md)를 참조하십시오.

## 어떻게 작동합니까?

[!UICONTROL 디바이스에서 의사 결정]을(를) 사용하여 [!DNL Adobe Target] SDK을 설치하고 초기화하면 서버와 가장 가까운 Akamai CDN에서 *규칙 아티팩트*&#x200B;가 서버에 로컬로 다운로드되고 캐시됩니다. 서버측 응용 프로그램 내에서 [!DNL Adobe Target] 경험을 검색하도록 요청하면 캐시된 규칙 아티팩트에 인코딩된 메타데이터를 기반으로 반환할 콘텐츠에 대한 결정이 메모리 내에서 이루어지는데, 이 메타데이터는 모든 [!UICONTROL 온디바이스 의사 결정] A/B 및 XT 활동을 정의합니다.

다음 다이어그램은 [!UICONTROL 온디바이스 의사 결정] 아키텍처를 보여 줍니다. 이미지를 확장하려면 를 클릭합니다.
...
* **지연 시간이 거의 0에 가까운 결정을 제공합니다.** 네트워크 요청이 차단되지 않도록 메모리 내 및 디바이스에서 버킷팅 및 의사 결정이 수행됩니다.
...
## [!UICONTROL 온디바이스 의사 결정]을 사용하도록 클라이언트를 프로비저닝하려면 어떻게 합니까?

[!DNL Adobe Target] 서버측 SDK를 사용하는 모든 [!DNL Adobe Target] 고객에게 온디바이스 의사 결정을 사용할 수 있습니다. 이 기능을 사용하려면 [!DNL Adobe Target] UI에서 **[!UICONTROL 관리]** > **[!UICONTROL 구현]** > **[!UICONTROL 계정 세부 정보]**(으)로 이동하여 **[!UICONTROL 장치 내 의사 결정]** 전환을 사용하도록 설정하십시오.

>[!NOTE]
>
>[!UICONTROL 디바이스에서 의사 결정] 전환을 활성화하거나 비활성화하려면 관리자 또는 승인자 *사용자 역할*&#x200B;이(가) 있어야 합니다.
...
>[!NOTE]
>
>[!UICONTROL 온디바이스 의사 결정]을 사용하도록 [!DNL Adobe Target] SDK을 초기화하기 전에 토글을 활성화하십시오. [!UICONTROL 온디바이스 의사 결정]이 작동하려면 먼저 규칙 아티팩트가 생성되어 Akamai CDN으로 전파되어야 합니다.

### 아티팩트 토글 시 기존의 모든 [!UICONTROL 디바이스에서 의사 결정] 적격 활동을 포함합니다.

[!UICONTROL 디바이스에서 의사 결정]을 사용할 수 있는 모든 라이브 [!DNL Target] 활동을 아티팩트에 자동으로 포함시키려면 이 **on**&#x200B;을(를) 전환하십시오.

이 토글을 **끄기**&#x200B;로 두면 생성된 규칙 아티팩트에 포함할 [!UICONTROL 디바이스에서 의사 결정] 활동을 다시 만들고 활성화해야 합니다.

## 활동이 [!UICONTROL 디바이스에서 의사 결정]을 수행할 수 있는지 어떻게 알 수 있습니까?

활동을 만든 후 활동 세부 정보 페이지에 표시되는 **[!UICONTROL Decisioning 메서드]** 레이블은 활동이 [!UICONTROL 온디바이스 의사 결정]을 사용할 수 있는지 여부를 나타냅니다.

![대체 이미지](assets/asset-odd9.png)

**[!UICONTROL Decisioning 메서드]** 열을 활동 목록에 추가하여 **[!UICONTROL 활동]** 페이지에서 [!UICONTROL 디바이스에서 의사 결정]할 수 있는 모든 활동을 볼 수도 있습니다.

![대체 이미지](assets/asset-odd7.png)

>[!NOTE]
>
>[!UICONTROL 디바이스에서 의사 결정]이 가능한 활동을 만들고 활성화한 후 Akamai CDN PoPs로 생성 및 전파되는 규칙 아티팩트에 포함되려면 20분이 걸릴 수 있습니다.

## [!UICONTROL 디바이스에서 의사 결정] 활동이 [!DNL Adobe Target]의 서버측 SDK을 통해 성공적으로 전달되도록 하려면 따라야 할 단계에 대한 요약은 무엇입니까?

1. [!DNL Adobe Target] UI에 액세스하여 **[!UICONTROL 관리]** > **[!UICONTROL 구현]** > **[!UICONTROL 계정 세부 정보]**(으)로 이동하여 **[!UICONTROL 디바이스에서 의사 결정]** 전환을 활성화합니다.
1. **[!UICONTROL 아티팩트에 기존의 모든 [!UICONTROL 디바이스에서 의사 결정] 적격 활동을 포함합니다]** 전환을 사용합니다.
1. [!UICONTROL 온디바이스 의사 결정]에서 지원하는 활동 유형을 만들고 활성화한 다음 해당 활동에 대한 **[!UICONTROL 의사 결정 메서드]**&#x200B;가 **[!UICONTROL 온디바이스 의사 결정]**&#x200B;인지 확인하십시오.
1. `decisioningMethod = on-device`을(를) 사용하여 [Node.js](../../node-js/overview.md) 또는 [Java](../../java/overview.md) SDK을 설치하고 초기화합니다.
...
[!DNL Adobe Target] [!UICONTROL 디바이스에서 의사 결정]을 통해 거의 0에 가까운 지연 시간 콘텐츠 배달을 사용할 수 있습니다.

이 7분 길이의 비디오:

* [!DNL Target] 구현의 다른 메서드와 비교하는 방법을 포함하여 [!UICONTROL 디바이스에서 의사 결정]에 대해 설명합니다.
* Target에서 [!UICONTROL 온디바이스 의사 결정]을 활성화하는 방법을 보여 줍니다.
* JSON 콘텐츠로 구성된 샘플 양식 기반 작성기 활동을 검사합니다
* [!UICONTROL 온디바이스 의사 결정]에 필요한 키 구성이 포함된 샘플 Node.JS SDK 코드를 표시합니다.
* 브라우저에서 결과를 보여 줍니다

>[!VIDEO](https://video.tv.adobe.com/v/329032/?quality=12)

자세한 비디오 및 튜토리얼은 [[!DNL Adobe Target] 튜토리얼](https://experienceleague.adobe.com/docs/target-learn/tutorials/overview.html?lang=ko-KR)을 참조하세요.
