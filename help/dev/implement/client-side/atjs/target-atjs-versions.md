...
description: 각 버전의 변경 사항에 대한 세부 사항 보기 [!DNL Adobe Target] at.js JavaScript 라이브러리입니다.
...
* 응답 토큰이 전송되지 않던 문제를 수정했습니다. `at-content-rendering-failed` 이벤트.
...
* 사용자 지정 설정에 대한 지원을 추가했습니다. [!DNL Adobe Experience Platform] (AEP) `sandboxId` 및 `sandboxName` 위치: `targetGlobalSettings`, 게재 API로 전달됨 `getOffer/getOffers` 호출.
* 체인화를 위한 섀도 DOM 수정 `:eq()` 선택기에서.
...
* 을(를) 제거함 `reactor-window` 및 `reactor-document` 다음을 보유한 고객에게 Platform launch 빌드가 올바르게 작동하도록 하는 Adobe Experience Platform Launch 모듈 `window.default` 또는 `document-default` 설정.
* at.js 1.8.3은 이제 명시적으로 `Samesite=None` 및 `Secure` 타사 도메인 쿠키가 제대로 설정되었는지 확인합니다.
...
* mbox 클릭 추적 및 A4T와 관련된 문제가 해결되었습니다. 0n-click으로 [!DNL Target] 올바른 mbox 및 mbox 매개 변수를 사용하여 배달 API 호출을 실행했습니다. 그러나 SDID가 의 SDID와 일치하지 않습니다 [!DNL Analytics] 를 호출했기 때문에 히트 결합과 전환이 없었습니다. (TNT-38372)
...
* 게재됨 적용 시 페이지 DOM에 추가되는 SCRIPT 및 STYLE 태그의 콘텐츠 보안 정책 임시 설정을 지원합니다. [!DNL Target] 오퍼. 고객이 설정할 수 있음 `targetGlobalSettings.cspScriptNonce` 및 `targetGlobalSettings.cspStyleNonce` at.js가 적용된 오퍼에 해당 스크립트 및 스타일 태그 주석을 설정할 수 있도록 합니다. 다음을 참조하십시오  [targetGlobalSettings](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md) 을 참조하십시오.
...
* 클릭 추적이 에서 전환을 보고하지 않던 문제를 수정했습니다. [!DNL Analytics for Target] (A4T) [!DNL Adobe Analytics] 코드가 페이지 요소에 없습니다.
...
* **클라이언트 측 분석 로깅**: 고객에게 분석 데이터를 전송하는 방법을 완벽하게 제어 [!DNL Adobe Analytics]클라이언트 측이든 아니면 서버측이든 상관없이 사용할 수 있습니다.

  자세한 내용은 [클라이언트측 [!DNL Analytics] 로깅](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/before-implement.html#client-side).
...
* 보기가 렌더링되었다는 알림이 클릭 추적 이벤트 핸들러에서 분리되었습니다. 처음에는, [!DNL Target] 렌더링된 보기에 속하는 클릭 이벤트 핸들러를 첨부할 수 없는 경우 알림을 보내지 않았습니다. [!DNL Target] 이제 클릭 요소를 찾을 수 없는 경우에도 보기 알림을 보냅니다. (TNT-31969)
...
>[!NOTE]
>
>에 대한 Adobe 옵트인 지원이 필요한 경우 [일반 데이터 보호 규정](/help/dev/before-implement/privacy/cmp-privacy-and-general-data-protection-regulation.md) (GDPR) 현재 at.js 1.7.0 또는 at.js 2.1.0 이상을 사용해야 합니다.
...
또한 이 릴리스에서는 다음과 같은 문제가 해결됩니다. [!DNL Target] 는 리디렉션 URL에서 발생하는 매개 변수로 리디렉션 URL 매개 변수를 무시할 수 있습니다.
...
* 을 사용할 때 리디렉션과 함께 모든 경험이 전달되지 않던 문제를 수정했습니다 [!DNL Analytics for Target] (A4T). (TNT-30444)
* 다음 이후에 페이지가 숨겨지는 문제가 해결되었습니다. [!DNL Target] 호출이 성공했습니다. (TNT-30358)
...
* `screenOrientation`, `devicePixelRatio` 및 `webGLRenderer`에 대한 지원을 추가했습니다. 이 새로운 항목 [!DNL Target] 요청 매개 변수는 iPhone X 및 기타 최신 장치 검색에 사용됩니다. 자세한 내용은 [모바일](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/categories-audiences/mobile.html)을 참조하십시오. (TNT-29781)
...
* 이제 at.js 요청은 GET을 사용하지만 URL 크기가 2048자를 초과하면 POST로 전환됩니다. 필요한 경우 크기 제한을 늘릴 수 있는 `urlSizeLimit`라는 새 특성이 있습니다. 이 변경을 통해 [!DNL Target] 를 사용하여 at.js를 AppMeasurement에 정렬합니다. 이 정렬에서는 동일한 기술을 사용합니다.
* [!DNL Target] 이제 다음을 시행합니다. `mbox` 의 키 `adobe.target.applyOffer(options)` 함수가 사용됩니다. 이 키는 과거에 필요했지만 [!DNL Target] 이제 을(를) 확인하기 위해 사용을 시행합니다. [!DNL Target] 은 적절한 유효성 검사를 수행하므로 고객은 함수를 올바르게 사용하고 있습니다.
...
* 이제 페이지가 백그라운드 탭에서 열릴 때도 at.js가 오퍼를 렌더링할 수 있습니다. 일부 [!DNL Target] 다음과 같은 경우 고객에게 문제가 발생했습니다. `requestAnimationFrame()` 백그라운드 탭의 브라우저 조절 동작으로 인해 이(가) 비활성화되었습니다.
...
* 을(를) 로드합니다 [!DNL Target] 라이브러리가 비동기적으로 표시됩니다. 자세한 내용은 [at.js FAQ](/help/dev/implement/client-side/atjs/target-atjs-faq.md)를 참조하십시오.
...
* 를 사용하는 경우 [!DNL Adobe Analytics] mbox.js 버전 61 이상 또는 at.js 버전 0.9.1 이상을 사용하는 경우 활동의 보고 소스로 활동 생성 중에 추적 서버를 지정할 필요가 없습니다. at.js 라이브러리는 [!DNL Target]에 추적 서버 값을 자동으로 전송합니다. 활동을 생성하는 동안에는 목표 및 설정 페이지의 추적 서버 필드를 비워둘 수 있습니다.
...
* 새 `targetGlobalSettings()` API 메서드.
  ...
  에서 구성하는 대신 at.js 라이브러리에서 설정을 재정의할 수 있습니다. [!DNL Target Standard/Premium] UI 또는 REST API 사용.
