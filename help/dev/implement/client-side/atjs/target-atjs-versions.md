...
>[!IMPORTANT]
>
>[!DNL Adobe Target]은(는) at.js 1.*x*&#x200B;와(과) at.js 2.*x*&#x200B;을(를) 모두 지원합니다.
>
>at.js 1.*x*&#x200B;이(가) 유지 관리 모드로 전환되었습니다. [!DNL Target] 팀은 필요한 경우 버그 수정 및 보안 패치를 릴리스합니다.
>
>[!DNL Target] 팀은 at.js 2.*x*&#x200B;을(를) 완벽하게 지원하고 버그 수정, 보안 패치, 기능 및 성능 최적화를 지속적으로 릴리스합니다.
>
...
* `prefetch`개의 요청을 사용할 때 [!UICONTROL Analytics for Target]&#x200B;(A4T) 클릭 메트릭 세부 사항이 올바르게 반환됩니다.
...
* CNAME 및 에지 재정의를 사용할 때 at.js 1.*x*&#x200B;이(가) 서버 도메인을 잘못 만들어 [!DNL Target] 요청이 실패하는 문제를 해결했습니다. (TNT-35064)
...
* CNAME 및 에지 재정의를 사용할 때 at.js 2.*x*&#x200B;이(가) 서버 도메인을 잘못 만들어 [!DNL Target] 요청이 실패하는 문제를 해결했습니다. (TNT-35065)
* [!DNL Target] 확장 v2 및 [!UICONTROL Adobe Analytics Launch] 확장을 사용할 때 [!DNL Target]에서 [!DNL Analytics] `sendBeacon` 호출을 지연시키는 문제를 해결했습니다. (TNT-36407, TNT-35990, TNT-36000)
...
* at.js 버전 2.2에서는 `serverState`이라는 새 설정도 제공합니다. 이 설정은 [!DNL Target]의 하이브리드 통합이 구현될 때 페이지 성능을 최적화하는 데 사용할 수 있습니다. 하이브리드 통합은 클라이언트측에서 at.js v2.2+를 사용하고 있으며, 서버측에서 배달 API 또는 [!DNL Target] SDK을 모두 사용하고 있음을 의미합니다. `serverState` 는 at.js v2.2+에서 서버측에서 가져온 콘텐츠에서 직접 경험을 적용하고 서비스되는 페이지의 일부로 클라이언트에 반환할 수 있는 기능을 제공합니다. 자세한 내용은 [targetGlobalSettings](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md#serverstate)의 &quot;serverState&quot;를 참조하십시오.
...
>[!WARNING]
>
>또한 at.js 버전 1.6.2에는 at.js 버전 1.6.1 및 1.6.0에 포함된 모든 개선 사항 및 수정 사항이 포함되어 있습니다. 이러한 버전은 더 이상 다운로드할 수 없습니다. 1.6.1 또는 1.6.0을 사용하는 경우 버전 1.6.2로 업그레이드하는 것이 좋습니다
...
* 찾아보기와 작성 간을 전환할 때 VEC 내에서 [!UICONTROL 경험 타깃팅] 활동 설정이 실패하는 문제가 해결되었습니다. (TNT-27942)
...
* 쿠키를 저장할 때 사용해야 하는 최상위 도메인을 추출하는 알고리즘이 at.js 버전 0.9.6에서 변경되었습니다. 이러한 변경으로 인해 쿠키는 IP를 사용하는 주소에 저장할 수 없습니다. 대부분의 경우 IP 주소는 테스트 용도로 사용되지만, 해결 방법으로 DNS 항목을 사용하거나, 로컬 상자에서 호스트 파일을 조정할 수 있습니다.
...
* 새 at.js 알림 API.
  ...
  이 알림 시스템의 목표는 페이지에서 at.js가 수행하는 작업과 문제가 발생하는 경우 더 많은 insight을 제공하는 것입니다. VEC에서 나타나는 일반적인 문제는 IT 릴리스가 페이지를 변경하고, VEC 선택기가 중단되고, 테스트가 더 이상 콘텐츠를 올바르게 배달하지 못하는 것입니다. 이 알림 시스템의 목표는 이 배달 문제를 페이지에 알려서 개발자들이 이 정보에 액세스하고, [!DNL Adobe Analytics]와 같은 시스템에 전달하고, 테스트가 중단된 비즈니스 소유자에게 경고를 보낼 수 있도록 하는 것입니다.
