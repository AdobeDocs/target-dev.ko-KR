...
description: 사용 [!UICONTROL adobe.target.applyOffers()] 함수 [!DNL Adobe Target] at.js JavaScript 라이브러리를 사용하여 응답에 여러 오퍼를 적용할 수 있습니다. (at.js 2.x)
title: 사용 방법 [!UICONTROL adobe.target.applyOffers()] 기능?
feature: at.js
...
| 키 | 유형 | 필수? | 설명 |
| --- | --- | --- | --- |
| selector | 문자열 | 아니오 | [!DNL Target]이 오퍼 컨텐츠를 배치해야 하는 HTML 요소를 식별하는 데 사용되는 HTML 요소 또는 CSS 선택기입니다. 선택기가 제공되지 않으면 [!DNL Target] 사용할 HTML 요소를 HTML HEAD으로 가정합니다. |
...
| response > prefetch > views > analytics > payload | [!DNL Analytics] 에 전송해야 하는 클라이언트측 통합을 위한 페이로드 [!DNL Analytics] 를 클릭합니다. |
...
| response > execute > mboxes > mbox > analytics > payload | [!DNL Analytics] 에 전송해야 하는 클라이언트측 통합을 위한 페이로드 [!DNL Analytics] mbox가 적용된 후. (A4T 사용 &quot;캠페인&quot; 섹션을 참조하십시오.) |
...
| response > execute > pageLoad > analytics > payload | [!DNL Analytics] 에 전송해야 하는 클라이언트측 통합을 위한 페이로드 [!DNL Analytics] 페이지 로드 콘텐츠가 적용된 후에. (A4T 사용 &quot;캠페인&quot; 섹션을 참조하십시오.) |

## 예 [!UICONTROL applyOffers()] 호출
...
## 다음을 포함하는 Promise 체인 호출 예 `[!UICONTROL getOffers()]` 및 `[!UICONTROL applyOffers()]`, 이는 이 함수들이 약속 기반이기 때문입니다
...
getOffers() 사용 방법에 대한 자세한 예는 getOffers 를 참조하십시오. [설명서](adobe-target-getoffers-atjs-2.md)
