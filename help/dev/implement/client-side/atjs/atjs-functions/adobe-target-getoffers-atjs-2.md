...
description: 사용 [!UICONTROL adobe.target.getOffers()] 함수 및 해당 옵션 [!DNL Adobe Target] at.js 라이브러리를 실행하여 여러 요청을 가져옵니다. [!DNL Target] 오퍼. (at.js 2.x)
title: 사용 방법 [!UICONTROL adobe.target.getOffers()] 기능?
feature: at.js
...
| 키 | 유형 | 필수? | 설명 |
| --- | --- | --- | --- |
| `consumerId` | 문자열 | 아니오 | 기본값이 제공되지 않을 경우 기본값은 클라이언트의 글로벌 mbox입니다. 이 키는 A4T 통합에 사용되는 SDID(Supplemental Data ID)를 생성하는 데 사용됩니다.<P>사용 시 `getOffers()`, 각 호출은 새 SDID를 생성합니다. 동일한 페이지에 여러 개의 mbox 요청이 있고 SDID를 유지하려는 경우(target-global-mbox의 SDID와 일치하도록) [!DNL Adobe Analytics] SDID), `consumerId` 매개 변수.<P>If `getOffers()` 에는 &quot;mbox1&quot;, &quot;mbox2&quot; 및 &quot;mbox3&quot;으로 명명된 세 개의 mbox가 포함되어 있습니다. `consumerId: "mbox1, mbox2, mbox3"` 다음에서 `getOffers()` 호출합니다. |
...
>[!NOTE]
>
>다음을 참조하십시오. [배달 API 설명서](/help/dev/implement/delivery-api/overview.md) 아래 나열된 모든 필드에 허용되는 유형에 대한 정보입니다.
...
| Request > experienceCloud > analytics > logging | 아니오 | 페이지에서 다음을 구현해야 합니다.<ul><li>방문자 ID 서비스</li><li>Appmeasurement.js</li></ul> | 지원되는 값은 다음과 같습니다.<P>**client_side**: 이 매개 변수를 지정하면 를 전송하는 데 사용해야 하는 호출자에게 분석 페이로드가 반환됩니다 [!UICONTROL Adobe Analytics] 를 통해 [!UICONTROL 데이터 삽입 API].<P>**server_side**[!DNL Target][!DNL Analytics]:  및 백엔드가 보고 목적으로 SDID를 사용하여 호출을 함께 연결하는 기본값입니다. |
| request > prefetch | 아니오 |  |  |
| request > prefetch > views | 아니오 | 최대 개수 50.<P>이름은 비워둘 수 없습니다.<P>이름 길이 `<=` 128.<P>값 길이 `<=` 5000.<P>이름은 &quot;profile&quot;로 시작하면 안 됩니다.<P>허용되지 않는 이름: &quot;orderId&quot;, &quot;orderTotal&quot;, &quot;productPurchasedId&quot;. | 활성 활동에서 적절한 보기를 검색하는 데 사용할 매개 변수를 전달합니다. |
| request > prefetch > views > profileParameters | 아니오 | 최대 개수 50.<P>이름은 비워둘 수 없습니다.<P>이름 길이 `<=` 128.<P>값 길이 `<=` 5000.<P>문자열 값만 허용합니다.<P>이름은 &quot;profile&quot;로 시작하면 안 됩니다. | 활성 활동에서 적절한 보기를 검색하는 데 사용할 프로필 매개 변수를 전달합니다. |
...
| request > prefetch > views > order > purchasedProductIds | 아니오 | 빈 값이 없습니다.<P>각 값의 최대 길이는 50자입니다.<P>쉼표로 연결 및 구분됩니다.<P>제품 ID 총 길이 `<=` 250 | 활성 활동에서 적절한 보기를 검색하는 데 사용할 구입 제품 ID를 전달합니다. |
| request > execute | 아니오 |  |  |
| request > execute > pageLoad | 아니오 |  |  |
| request > execute > pageLoad > parameters | 아니오 | 최대 개수 50.<P>이름은 비워둘 수 없습니다.<P>이름 길이 `<=` 128.<P>값 길이 `<=` 5000.<P>문자열 값만 허용합니다.<P>이름은 &quot;profile&quot;로 시작하면 안 됩니다.<P>허용되지 않는 이름: &quot;orderId&quot;, &quot;orderTotal&quot;, &quot;productPurchasedId&quot;. | 페이지가 로드될 때 지정된 매개 변수로 오퍼를 검색합니다. |
...
| request > execute > pageLoad > order > purchasedProductIds | 아니오 | 빈 값이 없습니다.<P>각 값의 최대 길이는 50자입니다.<P>쉼표로 연결 및 구분됩니다.<P>제품 ID 총 길이 `<=` 250 | 페이지가 로드될 때 지정된 구입 제품 ID로 오퍼를 검색합니다. |
...
| request > execute > mboxes > mbox > parameters | 아니오 | 최대 개수 = 50.<P>이름은 비워둘 수 없습니다.<P>이름 길이 `<=` 128.<P>문자열 값만 허용합니다.<P>값 길이 `<=` 5000.<P>이름은 &quot;profile&quot;로 시작하면 안 됩니다.<P>허용되지 않는 이름: &quot;orderId&quot;, &quot;orderTotal&quot;, &quot;productPurchasedId&quot;. | 지정된 매개 변수를 사용하여 주어진 mbox에 대한 오퍼를 검색합니다. |
...
| request > execute > mboxes > mbox > order > purchasedProductIds | 아니오 | 빈 값이 없습니다.<P>각 값의 최대 길이 = 50.<P>쉼표로 연결 및 구분됩니다.<P>제품 ID 총 길이 `<=` 250 | 지정된 주문 구입 제품 ID를 사용하여 주어진 mbox에 대한 오퍼를 검색합니다. |

## 호출 [!UICONTROL getOffers()] 모든 보기에 대해
...
## 호출 [!UICONTROL getOffers()] 온디바이스 의사 결정을 내리다
...
## 호출 [!UICONTROL getOffers()] 전달된 매개 변수와 프로필 매개 변수를 사용하여 최신 보기를 검색하려면
...
## 호출 [!UICONTROL getOffers()] 전달된 매개 변수와 프로필 매개 변수를 사용하여 mbox를 검색합니다.
...
## 호출 [!UICONTROL getOffers()] 클라이언트측에서 analytics 페이로드를 검색하려면
...
그런 다음 페이로드를 로 전달할 수 있습니다. [!DNL Adobe Analytics] 를 통해 [데이터 삽입 API](https://github.com/AdobeDocs/analytics-1.4-apis/blob/master/docs/data-insertion-api/index.md).

## 를 통해 여러 mbox에서 데이터 가져오기 및 렌더링 [!UICONTROL getOffers()] 및 [!UICONTROL applyOffers()]
...
## 호출 [!UICONTROL getOffers()] 페이지 로드를 수행하려면

다음 예에서는 을 사용하여 pageLoad를 수행하는 방법을 보여 줍니다 [!UICONTROL getOffers()] at.js 2.*x*
