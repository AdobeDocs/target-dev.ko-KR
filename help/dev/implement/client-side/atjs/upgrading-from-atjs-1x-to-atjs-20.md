...
description: 에서 업그레이드하는 방법 알아보기 [!DNL Adobe Target] at.js 1.x에서 at.js 2.x로 변경되었습니다. 시스템 흐름 다이어그램을 검사하고, 새로운 함수 및 더 이상 사용되지 않는 함수에 대해 알아봅니다.
...
* 간단한 1줄의 코드 및 일회용 개발자 설정으로 마케터가 만들어 실행할 수 있도록 할 수 있습니다. [!UICONTROL A/B 테스트] 및 [!UICONTROL 경험 타기팅] (XT) SPA에서 VEC를 통해 활동을 만듭니다.

## at.js 2.*x 시스템 다이어그램*

다음 다이어그램은 보기가 있는 at.js 2.*x*&#x200B;의 워크플로우를 이해하고 이를 통해 어떻게 SPA 통합이 향상되는지를 이해하는 데 도움이 됩니다. at.js 2.*x*&#x200B;에서 사용되는 개념의 도입을 보다 잘 이해하려면 [단일 페이지 애플리케이션 구현](/help/dev/implement/client-side/atjs/how-to-deployatjs/target-atjs-single-page-application.md)을 참조하십시오.
...
| 4 | 프로필 스크립트가 실행된 다음 프로필 저장소에 반영됩니다. 저장소는 대상 라이브러리의 적절한 대상(예: [!DNL Adobe Analytics], [!DNL Audience Manager] 등에서 공유되는 대상)을 요청합니다.<P>고객 속성은 묶음 프로세스를 통해 프로필 저장소로 전송됩니다. |
| 5 | [!DNL Target]에서는 URL 요청 매개변수 및 프로필 데이터를 기반으로 현재 페이지 및 미래 보기를 위해 방문자에게 반환할 활동 및 경험을 결정합니다. |
| 6 | 타기팅된 콘텐츠는 다시 페이지로 전송되며, 원할 경우 추가적인 개인화를 위한 프로필 값을 포함할 수 있습니다.<P>현재 페이지의 타기팅된 콘텐츠는 기본 콘텐츠의 플리커 없이 가능한 한 빨리 나타납니다.<P>`triggerView()`를 통해 보기를 트리거할 때 추가적인 서버 호출 없이 즉시 적용할 수 있도록 브라우저에서 캐시된 SPA의 사용자 동작에 대한 결과로서 표시되는 보기를 위한 타깃팅된 콘텐츠입니다. |
| 7 | [!UICONTROL Analytics 데이터가 데이터 수집 서버로 전송됩니다.] |
| 8 | 타겟팅된 데이터가 다음과 일치함 [!UICONTROL 분석] SDID를 통해 전송되는 데이터는에서 [!UICONTROL 분석] 보고 스토리지입니다.<P>그런 다음 [!UICONTROL Analytics] 데이터는 [!UICONTROL Analytics for ] (A4T) 보고서를 통해 [!DNL Target]Analytics[!UICONTROL  및 ]Target 모두에서 볼 수 있게 됩니다. |
...
| 5 | [!UICONTROL Analytics 데이터가 데이터 수집 서버로 전송됩니다.] |
| 6 | [!DNL Target] 데이터는 SDID를 통해 Analytics 데이터에 대응되며 Analytics 보고 저장소로 처리됩니다.  [!UICONTROL 그런 다음 Analytics 데이터는 A4T 보고서를 통해 Analytics 및 모두에서 볼 수 있게 됩니다.][!DNL Target] |

## at.js 2.*x*

at.js 2.*x* 의 태그를 통해 [Adobe Experience Platform](/help/dev/implement/client-side/atjs/how-to-deployatjs/implement-target-using-adobe-launch.md) 확장명.
...
>또는
>
>at.js 2.*x* 사용 [!DNL Target] UI 및 를 사용하여 배포 [선택한 방법](/help/dev/implement/client-side/atjs/how-to-deployatjs/how-to-deployatjs.md).
...
| 메서드 | 지원됨? | 신규? | 삭제 예정?<P>(기본 콘텐츠가 표시됩니다.) |
| --- | --- | --- | --- |
| `getOffer()` | 예 |  |  |
| `getOffers()` |  | 예 |  |
| `applyOffer()` | 예 |  |  |
...
at.js 2.*x에서는 배달 API라고 하는 새 API를 사용합니다.* at.js에서 [!DNL Target] edge server가 올바르면 브라우저의 개발자 도구에 있는 네트워크 탭을 &quot;게재&quot;, &quot;로 필터링할 수 있습니다.`tt.omtrdc.net`,&quot; 또는 클라이언트 코드입니다. 또한 [!DNL Target]에서 키-값 쌍 대신 JSON 페이로드를 전송하는 것을 보게 됩니다.

### [!DNL Target] 글로벌 mbox가 더 이상 사용되지 않음

at.js 2.*x*, 더 이상 &quot;`target-global-mbox`&quot; 네트워크 호출에서 볼 수 있습니다. 대신 &quot; &quot;을(를) 교체했습니다.`target-global-mbox`&quot; 구문을 &quot; &quot;`execute > pageLoad`에 전송된 JSON 페이로드에서 &quot; [!DNL Target] 아래에 표시된 서버:
...
고객은 를 통해 글로벌 mbox 이름을 지정할 수 있습니다. **[!UICONTROL Target]** > **[!UICONTROL 관리]** > **[!UICONTROL 구현]** > **[!UICONTROL at.js 설정 편집]**. 이 설정은 [!DNL Target] Edge Server에서 execute > pageLoad를 [!DNL Target] UI에 나타나는 글로벌 mbox 이름으로 변환하는 데 사용됩니다. 이렇게 변환하면 고객은 계속해서 서버 측 API, 양식 기반 작성기, 프로필 스크립트를 사용할 수 있으며 글로벌 mbox 이름을 사용하여 대상을 만들 수 있습니다. 또한 동일한 글로벌 mbox 이름이 **[!UICONTROL 관리]** > **[!UICONTROL 시각적 경험 작성기]** 또한 at.js 1을 사용하는 페이지가 여전히 있는 경우 페이지도 표시합니다.*x*&#x200B;를 참조하십시오.
...
### at.js 2.x 에서 도메인 간 추적 지원

도메인 간 추적을 통해 여러 도메인 간에 방문자를 연결할 수 있습니다. 각 도메인에 대해 새 쿠키를 만들어야 하기 때문에 방문자가 도메인에서 도메인으로 이동할 때 추적하기가 어렵습니다. 도메인 간 추적을 수행하기 위해 [!DNL Target]은 타사 쿠키를 사용하여 도메인 간에 방문자를 추적합니다. 이를 통해 다음을 만들 수 있습니다. [!DNL Target] 다음 범위에 걸쳐 수행되는 활동 `siteA.com` 및 `siteB.com` 방문자가 고유한 도메인을 탐색할 때도 동일한 경험을 유지합니다. 이 기능은 [!DNL Target]의 타사 및 퍼스트 파티 쿠키 동작과 연결되어 있습니다.

>[!NOTE]
>
>도메인 간 추적은 at.js 2.10부터 지원되지만 at.js 2.*x* 2.10 이전 도메인 간 추적은 at.js 2에서 지원됩니다.at.js 2.*x*&#x200B;에서 지원됩니다.

위치 [!DNL Target], 타사 쿠키가에 저장됩니다. `<CLIENTCODE>.tt.omtrdc.net`. 자사 쿠키가에 저장됩니다. `clientdomain.com`. 첫 번째 요청은 `mboxSession` 및 `mboxPC`라는 타사 쿠키를 설정하는 HTTP 응답 헤더를 반환하는 반면, 리디렉션 요청이 추가 매개 변수(`mboxXDomainCheck=true`)를 사용하여 다시 전송됩니다. 브라우저가 타사 쿠키를 수락하면 리디렉션 요청에 해당 쿠키가 포함되고 경험이 반환됩니다. 여기서는 HTTP GET 메서드를 사용하므로 이 워크플로우가 가능합니다.

단, at.js 2.*x*, HTTP GET은 사용되지 않습니다. 대신 HTTP POST은 at.js 2.*x*[!DNL Target]를 통해 Edge 서버에 JSON 페이로드를 전송하는데 사용됩니다. HTTP POST을 사용하면 브라우저가 서드파티 쿠키를 지원하는지 여부를 확인하는 리디렉션 요청이 중단됨을 의미합니다. 이것은 HTTP GET 요청은 멱등 트랜잭션이 아니지만 HTTP POST는 비멱등이어서 임의로 반복되면 안 되기 때문입니다. 따라서 at.js 2.*x* (2.10 이전 버전)은 기본적으로 지원되지 않습니다. at.js 1.*x*&#x200B;는 도메인 간 추적을 위한 기본 지원을 제공합니다.

at.js v2.10 이상에 대해 도메인 간 추적을 사용하려면 다음 중 하나를 수행할 수 있습니다.

1. 설치 [ECID 라이브러리 v4.3.0+](https://experienceleague.adobe.com/docs/id-service/using/release-notes/release-notes.html?lang=ko-KR) at.js 2.*x*. ECID 라이브러리는 도메인 간에 방문자를 식별하는 데 사용되는 영구 ID를 관리하기 위해 존재합니다. ECID library v4.3.0+ 및 at.js 2.*x*&#x200B;를 설치한 뒤 고유한 도메인을 확장하고 사용자를 추적할 수 있는 활동을 만들 수 있습니다. 이 기능은 세션이 만료된 후에만 작동합니다.

1. at.js v2.10 이상이 있는 경우 ECID 라이브러리를 설치하는 대신 [!DNL Target] 의 UI **[!UICONTROL 관리]** > **[!UICONTROL 구현]**. (또는 다음을 설정할 수 있습니다.) _도메인 간_ 옵션 대상 _활성화됨_ at.js 코드에서)

at.js v2 버전에 대해 도메인 간 추적을 사용하려면&#x200B;*x* 2.10 이전에는 위의 옵션 #1을 구현(ECID 라이브러리 설치)할 수 있습니다.
...
고객은 를 통해 글로벌 mbox 이름을 지정할 수 있습니다. **[!UICONTROL Target]** > **[!UICONTROL 관리]** > **[!UICONTROL 구현]** > **[!UICONTROL 편집]**. 이 설정은 [!DNL Target] Edge Server에서 execute > pageLoad를 입력된 글로벌 mbox 이름으로 변환하는 데 사용됩니다. 이렇게 변환하면 고객은 계속해서 서버 측 API, 양식 기반 작성기, 프로필 스크립트를 사용할 수 있으며 글로벌 mbox를 타깃팅하는 대상을 만들 수 있습니다.
...
### 전화하면 적혀있어요 `triggerView()` &amp;lbrace; 사용`"page" : "true"`&amp;rbrace;, 알림이 [!DNL Target] 를 업데이트하고 노출을 늘립니다. 이렇게 되면 프로필 스크립트도 실행됩니까?
...
### at.js 1.*x* 대상 생성을 위한 매개 변수는 at.js 2에서 지원되지 않습니다.*x*&#x200B;를 사용하는 SPA에서)하면 어떻게 될까요.

다음 at.js 1.x 매개 변수는 *아님* at.js 2를 사용할 때 현재 대상 만들기에 대해 지원됩니다.*x*:
...
### at.js 2.*x* 가 vst를 사용하여 대상자 만들기를 지원하지 않습니다.* 매개변수

at.js 1.*x* vst를 사용할 수 있습니다.* 대상을 만들 mbox 매개 변수입니다. 이것은 at.js 1.*x* mbox 매개 변수를 [!DNL Target] 백엔드. at.js 2로 마이그레이션한 후&#x200B;*x*, at.js 2.*x* mbox 매개 변수를 다르게 보냅니다.
...
>[!NOTE]
>
>[!UICONTROL 자동 Target] 활동은 at.js 2를 통해 지원됩니다.*x* 및 모든 수정 사항이 다음에 적용되는 VEC `Page Load Event`. 특정 보기에 수정 사항이 추가되면 [!UICONTROL A/B 테스트], [!UICONTROL 자동 할당], 및 [!UICONTROL 경험 타기팅] (XT) 활동만 지원됩니다.
...
| 옵트인 지원 [GDPR](/help/dev/before-implement/privacy/cmp-privacy-and-general-data-protection-regulation.md) | 다음에서 지원됩니다. [at.js 버전 2.1.0](/help/dev/implement/client-side/atjs/target-atjs-versions.md#atjs-version-210-june-3-2019) 나중에 |
| AEM Enhanced Personalization 제공 [!DNL Adobe Target] | 아니오 |
...
## at.js 1.*x* 매개 변수와 at.js 2 *X*&#x200B;의 페이로드 매핑
...
가 있는 페이지의 도메인 [!DNL Target] 라이브러리가 실행됩니다.
...
다른 항목을 연결하는 데 사용되는 고객 타사 ID [!DNL Target] ID
...
at.js 2.*x* Adobe 향상 [!DNL Target]SPA에 대한 의 지원 및 다른 Experience Cloud 솔루션과 통합됩니다. 다음 비디오에서는 모든 것이 어떻게 합쳐지는지 설명합니다.

>[!VIDEO](https://video.tv.adobe.com/v/26250/?quality=12)

다음을 참조하십시오 [at.js 2.*x* 작품](https://experienceleague.adobe.com/docs/target-learn/tutorials/implementation/understanding-how-atjs-20-works.html) 추가 정보.
