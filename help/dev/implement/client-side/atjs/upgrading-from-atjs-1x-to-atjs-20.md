...
# at.js 1.*x*&#x200B;에서 at.js 2.*x*&#x200B;으로 업그레이드
...
at.js 2.*x*&#x200B;을 사용하면 이전 버전에서 사용할 수 없는 다음과 같은 몇 가지 이점이 있습니다.
...
다음 다이어그램은 보기가 있는 at.js 2.*x*&#x200B;의 워크플로를 이해하고 이를 통해 어떻게 SPA 통합이 향상되는지를 이해하는 데 도움이 됩니다. at.js 2.*x*&#x200B;에서 사용되는 개념의 도입을 보다 잘 이해하려면 [단일 페이지 애플리케이션 구현](/help/dev/implement/client-side/atjs/how-to-deployatjs/target-atjs-single-page-application.md)을 참조하십시오.
...
[Adobe Experience Platform](/help/dev/implement/client-side/atjs/how-to-deployatjs/implement-target-using-adobe-launch.md) 확장의 태그를 통해 at.js 2.*x*&#x200B;을(를) 배포합니다.
...
>또는
>
>[!DNL Target] UI를 사용하여 at.js 2.*x*&#x200B;을(를) 수동으로 다운로드하고 선택한 [메서드를 사용하여 배포](/help/dev/implement/client-side/atjs/how-to-deployatjs/how-to-deployatjs.md)하십시오.

## 사용 중단된 at.js 함수

at.js 2.*x*&#x200B;에서 더 이상 사용되지 않는 몇 가지 함수가 있습니다.

>[!WARNING]
>
>at.js 2.*x*&#x200B;을(를) 배포할 때 이렇게 사용되지 않는 함수가 여전히 사이트에서 사용되면 콘솔 경고가 표시됩니다. 업그레이드할 때 권장되는 접근 방법은 스테이징 환경에서 at.js 2.*x*&#x200B;의 배포를 테스트하고 콘솔에 기록된 모든 경고를 하나하나 다 확인하고, 사용이 중단된 함수를 at.js 2.*x*&#x200B;에 도입된 새로운 함수로 변환하는 것입니다.
...
>[!NOTE]
>
>at.js 2.*x*&#x200B;은(는) `mboxDefault` 표시 요소를 더 이상 자동으로 사전에 숨기지 않습니다. 따라서 고객은 사이트에서 수동으로 또는 태그 관리자를 통해 사전 숨김 로직을 수용해야 합니다.
...
## 2.*x*&#x200B;에서 더 이상 사용되지 않는 함수, 새로운 함수 및 지원되는 at.js 함수에 대한 요약
...
### at.js 1.*x*&#x200B;이(가) 다른 페이지에 있는 동안 at.js 2.*x*&#x200B;을(를) 일부 페이지에서 사용할 수 있습니까?

예. 방문자 프로필은 서로 다른 버전 및 라이브러리를 사용하는 여러 페이지 간에 보존됩니다. 쿠키 형식은 동일합니다.

### at.js 2.*x*&#x200B;에서 새 API 사용

at.js 2.*x*&#x200B;에서는 배달 API라고 하는 새 API를 사용합니다. at.js가 [!DNL Target] Edge Server를 올바로 호출하는지를 디버깅하기 위해 브라우저의 개발자 도구에 있는 네트워크 탭을 &quot;배달&quot;, &quot;`tt.omtrdc.net`&quot; 또는 클라이언트 코드로 필터링할 수 있습니다. 또한 [!DNL Target]에서 키-값 쌍 대신 JSON 페이로드를 전송하는 것을 보게 됩니다.

### [!DNL Target] 글로벌 Mbox가 더 이상 사용되지 않습니다.

at.js 2.*x*&#x200B;에서 더 이상 네트워크 호출에 &quot;`target-global-mbox`&quot;이(가) 눈에 띄게 표시되지 않습니다. 대신 아래와 같이 [!DNL Target] 서버로 전송된 JSON 페이로드에서 &quot;`target-global-mbox`&quot; 구문을 &quot;`execute > pageLoad`&quot;(으)로 대체했습니다.
...
### 지원되는 at.js 2.*x* 설정과 지원되지 않는 at.js 2.
...
>[!NOTE]
>
>도메인 간 추적은 at.js 2.10부터 지원되지만, 2.10 이전의 at.js 2.*x*&#x200B;에서는 기본적으로 지원되지 않습니다. 도메인 간 추적은 ECID(Experience Cloud ID) 라이브러리 v4.3.0+를 통해 at.js 2.*x*&#x200B;에서 지원됩니다.
...
그러나 at.js 2.*x*&#x200B;에서는 HTTP GET이 사용되지 않습니다. 대신 at.js 2.*x*&#x200B;을 통해 HTTP POST를 사용하여 [!DNL Target] Edge 서버에 JSON 페이로드를 보냅니다. HTTP POST를 사용하면 브라우저가 타사 쿠키를 지원하는지 여부를 확인하는 리디렉션 요청이 중단됨을 의미합니다. 이것은 HTTP GET 요청은 멱등 트랜잭션이 아니지만 HTTP POST는 비멱등이어서 임의로 반복되면 안 되기 때문입니다. 따라서 at.js 2.*x*(2.10 이전)의 도메인 간 추적은 즉시 지원되지 않습니다. at.js 1.*x*&#x200B;만 도메인 간 추적을 위한 기본 지원을 제공합니다.
...
아니요. `triggerView()`를 호출하기 전에 사전 숨김(pre-hiding) 코드를 추가할 필요는 없습니다. at.js 2.*x*&#x200B;에서는 보기가 표시되고 적용되기 전에 사전 숨김 및 플리커(깜박임) 로직을 관리합니다.

### 대상을 만들기 위한 at.js 1.*x* 매개 변수는 at.js 2.*x*&#x200B;에서 지원되지 않습니까?

다음 at.js 1.x 매개 변수는 at.js 2.*x*&#x200B;을 사용할 때 현재 대상 만들기에 대해 *지원되지 않음*&#x200B;입니다.
...
### at.js 2.*x*&#x200B;에서는 vst.* 매개 변수를 사용하여 대상을 만들 수 없습니다.

at.js 1.*x*&#x200B;의 고객은 vst.* mbox 매개 변수를 사용하여 대상을 만들 수 있습니다. at.js 1.*x*&#x200B;에서 mbox 매개 변수를 [!DNL Target] 백 엔드로 보낸 방식에 대한 의도하지 않은 부작용입니다. at.js 2.*x*(으)로 마이그레이션한 후 at.js 2.*x*&#x200B;에서 mbox 매개 변수를 다르게 보내므로 이러한 매개 변수를 사용하여 대상을 더 이상 만들 수 없습니다.

## at.js 호환성

다음 테이블에서는 다양한 활동 유형, 통합, 기능 및 at.js 함수와의 at.js 2.*x* 호환성과 다양한 활동 유형, 통합, 기능 및 at.js 함수.
...
at.js 2.*x*&#x200B;은(는) at.js 1.*x*&#x200B;과(와) 마찬가지로 사용자 지정 이벤트 `at-request-succeeded`을(를) 사용하여 응답 토큰을 표시합니다. `at-request-succeeded` 사용자 지정 이벤트를 사용한 코드 예에 대해서는 [응답 토큰](https://experienceleague.adobe.com/docs/target/using/administer/response-tokens.html?lang=ko)을 참조하십시오.

## at.js 1.*x* 매개 변수를 at.js 2.*x* 페이로드 매핑에

이 섹션에서는 at.js 1.*x*&#x200B;과(와) at.js 2.*x* 간의 매핑에 대해 간략히 설명합니다.
...
* at.js 1.*x* - 클라이언트 코드는 경로의 일부입니다.
* at.js 2.*x* - 클라이언트 코드는 다음과 같은 쿼리 문자열 매개 변수로 전송됩니다.
  `http://<client code>.tt.omtrdc.net/rest/v1/delivery?client=democlient`

다음 섹션에는 각 at.js 1.*x* 매개 변수, 설명 및 해당 2.*x* JSON 페이로드(해당하는 경우)가 나열됩니다.
