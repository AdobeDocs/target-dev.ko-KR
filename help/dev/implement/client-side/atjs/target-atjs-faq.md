...
FP-11577을 사용하는 [!DNL Adobe Experience Manager] 6.2(또는 이상)에서는 이제 [!UICONTROL Adobe Target Cloud Services] 통합을 통해 at.js 구현을 지원합니다.
...
at.js 구현은 단일 라이브러리(at.js)를 사용하는 반면, mbox.js 구현은 실제로 두 개의 라이브러리(mbox.js 및 target.js)를 사용합니다. 따라서 공정한 비교는 at.js 대 mbox.js *와* `target.js`입니다. 두 버전의 압축 크기를 비교하면 at.js 버전 1.2는 34KB이고 mbox.js 버전 63은 26.2KB입니다. &quot;
...
예. [at.js 버전 1.2 이상](/help/dev/implement/client-side/atjs/target-atjs-versions.md)을 사용 중이라면 사용할 수 있습니다. 그러나 Adobe에서는 최신 버전을 사용하여 최신 상태로 유지하는 것이 좋습니다.
...
tt.omtrdc.net은 [!DNL Target]에 대한 모든 서버 호출을 받는 데 사용되는 Adobe의 EDGE 네트워크의 도메인 이름입니다.
...
TargetGlobalSettings() 함수([targetGlobalSettings](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md))에서 `secureOnly` 옵션이 true로 설정되어 있는 한, at.js에서 활성화한 Adobe Edge 네트워크와의 통신은 HTTPS를 통해서만 가능합니다. 그렇지 않으면 at.js는 페이지 프로토콜을 기준으로 HTTP와 HTTPS 간을 전환할 수 있습니다.
...
또한, Adobe Edge Network에 대한 요청은 공개적이며(방문자의 브라우저에서 수행되도록 설계되어 있으므로), 가시적인 방문자 세부 정보를 포함하지 않습니다(방문자 ID만 포함). 이러한 요청은 방문자에게 경험을 전달하며, 방문자가 페이지에서 봐야 하는 사항에 대한 세부 정보를 포함합니다.
