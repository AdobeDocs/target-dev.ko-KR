...
description: at.js 및 [!DNL Target] 페이지 또는 앱 로드 중 깜박임(활동 콘텐츠로 대체되기 전에 일시적으로 표시되는 기본 콘텐츠)을 방지합니다.
...
다음 방법에 대한 정보 [!DNL Adobe Target] at.js JavaScript 라이브러리는 페이지 또는 앱 로드 중 깜박임을 방지합니다.

플리커는 기본 컨텐츠가 활동 컨텐츠로 대체되기 전에 방문자에게 순간적으로 표시될 때 발생합니다. 플리커는 방문자에게 혼란을 야기할 수 있으므로 달갑지 않습니다.

## 자동 생성된 글로벌 mbox 사용

at.js 구성 시 [글로벌 mbox를 자동으로 만들기](/help/dev/implement/client-side/atjs/global-mbox/customize-global-mbox.md) 설정을 활성화하면 at.js가 페이지 로드 시 불투명도 설정을 변경하여 플리커(깜박임)를 관리합니다. at.js가 로드되면 at.js는 요소의 불투명도 설정을 `<body>`&quot;0&quot;으로 변경하여 방문자가 처음에 페이지를 볼 수 있도록 합니다. 의 응답 후 [!DNL Target] 수신 - 또는 [!DNL Target] 요청이 감지됨 - at.js는 불투명도를 &quot;1&quot;로 재설정합니다. 이렇게 하면 활동의 컨텐츠가 적용된 후에만 방문자에게 페이지가 표시됩니다.

at.js 구성 시 이 설정을 활성화할 경우 at.js가 HTML BODY 스타일 불투명도를 0으로 설정합니다. 의 응답 후 [!DNL Target] 이 수신되면 at.js는 HTML BODY 불투명도를 1로 재설정합니다.

0으로 설정된 불투명도는 깜박임을 방지하기 위해 페이지 컨텐츠를 숨기지만, 브라우저가 여전히 페이지를 렌더링하고 CSS, 이미지 등과 같은 필요한 모든 자산을 로드합니다.

If `opacity: 0` 은 구현에서 작동하지 않습니다. 를 사용자 지정하여 깜박임을 관리할 수도 있습니다 `bodyHiddenStyle` 및 설정 `body {visibility:hidden !important}`. 다음 중 하나를 사용할 수 있습니다 `body {opacity:0 !important}` 또는 `body {visibility:hidden !important}`, 그 중 특정 환경에 가장 적합한 것을 선택합니다.
...
![Target 흐름: 자동 생성된 글로벌 mbox](/help/dev/implement/client-side/atjs/how-atjs-works/assets/target-flow2.png "target 흐름: 자동 생성된 글로벌 mbox"){zoomable=&quot;yes&quot;}
