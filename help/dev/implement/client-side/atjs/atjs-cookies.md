...
`AMCV_###@AdobeOrg`개의 요청에서 `ECID`이(가) 전달되더라도 [!DNL Target] 퍼스트 파티 쿠키는 항상 Experience Cloud ID 서비스에 의해 설정됩니다.
