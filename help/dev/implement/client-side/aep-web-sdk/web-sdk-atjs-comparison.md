...
at.js 2.x를 사용하여 `pageLoadEnabled,` 설정을 사용하면 라이브러리가 `execute -> pageLoad`을(를) 사용하여 [!DNL Target] Edge에 대한 호출을 트리거합니다. 모든 설정이 기본값으로 설정되어 있으면 사용자 지정 코딩이 필요하지 않습니다. at.js가 페이지에 추가되고 브라우저에 의해 로드되면 [!DNL Target] Edge 호출이 실행됩니다.
...
`decisionScopes`: `__view__`에서 특수 범위로 `sendEvent` 명령을 실행합니다. [!DNL Adobe]은(는) 이 범위를 신호로 사용하여 [!DNL Target]에서 모든 페이지 로드 활동을 가져오고 모든 보기를 미리 가져옵니다. [!DNL Platform Web SDK]은(는) 또한 모든 VEC 보기 기반 활동을 평가하려고 합니다. 현재 [!DNL Platform Web SDK]에서는 보기 프리페치를 사용하지 않도록 설정할 수 없습니다.
...
`applyOffers` 함수를 사용하여 [!DNL Target] 활동을 적용할 수 있습니다. `adobe.target.applyOffer(options).`
...
[전용 설명서](https://experienceleague.adobe.com/en/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/adobe-target-applyoffers-atjs-2)에서 `applyOffers` 명령에 대해 자세히 알아보세요.

### [!DNL Platform Web SDK] 사용 중

`applyPropositions` 명령을 사용하여 [!DNL Target] 활동을 적용할 수 있습니다.
...
`_experience.decisioning.propositions` XDM `fieldgroup`은(는) 개체 배열입니다. 각 개체의 속성은 `sendEvent` 명령에서 반환되는 `result.propositions`에서 파생됩니다. `{ id, scope, scopeDetails }.`
...
[!DNL Analytics] 페이로드(`tnta` 토큰)는 [데이터 삽입 API](https://github.com/AdobeDocs/analytics-1.4-apis/blob/master/docs/data-insertion-api/index.md)를 사용하여 [!DNL Analytics] 히트에 포함해야 합니다.

#### [!DNL Analytics] 서버측 로깅

at.js 설정에서 `analyticsLogging: server_side`을(를) 설정하거나 `window.targetglobalSettings` 개체를 재정의하여 [!DNL Analytics] 서버측 로깅을 사용하도록 설정할 수 있습니다.
...
해당 DataStream 구성에 대해 [!DNL Adobe Analytics]을(를) 사용하지 않도록 설정하면 [!DNL Analytics] 클라이언트 측 로깅이 사용됩니다.
...
고객은 `sendEvent` 명령을 체인화하고 결과 제안 배열을 반복하여 [데이터 삽입 API](https://github.com/AdobeDocs/analytics-1.4-apis/blob/master/docs/data-insertion-api/index.md)를 사용하여 [!DNL Analytics]과(와) 공유해야 하는 [!DNL Analytics] 토큰(`tnta`)에 액세스할 수 있습니다.
...
[!DNL Target] UI에서 설정을 구성하거나 REST API를 사용하는 대신 `window.targetGlobalSettings,`을(를) 사용하여 at.js 라이브러리의 설정을 재정의할 수 있습니다.
...
[!DNL Target] 프로필을 업데이트하려면 `sendEvent` 명령을 사용하고 `profile.`을(를) 사용하여 키 이름 앞에 붙여 `data.__adobe.target` 속성을 설정합니다.
...
[!DNL Recommendations] 데이터를 보내려면 `sendEvent` 명령을 사용하고 `entity.`을(를) 사용하여 키 이름 앞에 붙여 `data.__adobe.target` 속성을 설정합니다.
...
`targetPageParams`을(를) 설정할 때 `pag-lLoad`이라고도 하는 `target-global-mbox`에 대한 요청을 보냅니다.

권장 사항은 [!DNL Target] 요청마다 전송되므로 `targetPageParamsAll`을(를) 사용하여 설정해야 합니다. `targetPageParamsAll`을(를) 사용하면 페이지에서 `mbox3rdPartyId`을(를) 한 번 정의하여 모든 [!DNL Target] 요청에 올바른 `mbox3rdPartyId.`이(가) 있는지 확인할 수 있다는 이점이 있습니다
...
이 기능은 at.js 2.x에서만 사용할 수 있습니다. at.js 2.x에는 `getOffers`(이)라는 새 함수가 있습니다. `getOffers` 함수를 사용하면 고객이 하나 이상의 mbox에 대한 콘텐츠를 미리 가져올 수 있습니다. 다음은 한 예입니다.
...
>[!NOTE]
>
>Adobe에서는 `mboxes` 배열의 모든 `mbox`에 자체 인덱스가 있는지 확인하는 것이 좋습니다. 일반적으로 첫 번째 mbox에는 `index=0`, 다음 mbox `index=1,` 등이 있습니다.
