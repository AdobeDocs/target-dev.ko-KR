...
[!UICONTROL 단일 프로필 업데이트 API] 및 는 일반적으로 [!DNL Target]을(를) 구현하지 않은 채널에서 발생하는 트랜잭션과 관련하여 업데이트가 필요한 경우에 사용됩니다. 예를 들어 일부 오프라인 작업을 수행하는 단일 방문자의 프로필을 업데이트하려고 합니다. 콜 센터 문의, 대출 지원, 매장 내 로열티 카드 사용, 키오스크 액세스 등의 작업을 수행할 수 있습니다.

[!UICONTROL 단일 프로필 업데이트 API]의 이점은 다음과 같습니다.
...
* [!UICONTROL 단일 프로필 업데이트 API]는 연속 24시간 동안 100만 개의 업데이트를 수행하도록 제한됩니다.
...
* [!UICONTROL 단일 프로필 업데이트 API]는 서버 간 API이며 웹 페이지에서 작동하도록 디자인되지 않았습니다. 웹 페이지 내에서 방문자 프로필을 업데이트하려면 [trackEvent()](/help/dev/implement/client-side/atjs/atjs-functions/adobe-target-trackevent.md) 함수 또는 [배달 API](/help/dev/implement/delivery-api/overview.md)를 사용할 수 있습니다.
...
[!UICONTROL 단일 프로필 업데이트 API]는 업데이트용입니다. 아무 것도 발견되지 않으면 프로필이 만들어지지 않습니다.
