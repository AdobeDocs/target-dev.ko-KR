...
## 타겟에 대한 Adobe Analytics(A4T)

서버에서 Target 배달 API 호출이 실행되면 Adobe Target은 해당 사용자에 대한 경험을 반환하며 그 외에도 Adobe Target은 Adobe Analytics 페이로드를 다시 호출자에게 반환하거나 자동으로 Adobe Analytics에 전달합니다. 서버측의 Adobe Analytics에 Target 활동 정보를 보내려면 다음과 같이 충족해야 하는 몇 가지 전제 조건이 있습니다.

1. 활동은 Adobe Target UI에서 Adobe Analytics을 보고 소스로 설정되며 계정은 A4T에 대해 활성화됩니다
1. Adobe Marketing Cloud 방문자 ID는 API 사용자에 의해 생성되며, Target 배달 API 호출이 실행될 때 사용할 수 있습니다
...
1. `trackingServer` - Adobe Analytics 서버 Adobe Target과 Adobe Analytics이 데이터를 올바르게 결합하기 위해 동일한 `supplementalDataId` Adobe Target과 Adobe Analytics 모두에 전달해야 합니다.
...
Adobe Target 배달 API 소비자는 해당 mbox에 대한 Adobe Analytics 페이로드를 검색할 수 있으므로 소비자가 를 통해 페이로드를 Adobe Analytics에 보낼 수 있습니다. [데이터 삽입 API](https://github.com/AdobeDocs/analytics-1.4-apis/blob/master/docs/data-insertion-api/index.md). 서버측 Adobe Target 호출이 실행되면 을 전달합니다 `client_side` (으)로 `logging` 요청의 필드. 이렇게 하면 Analytics를 보고 소스로 사용하는 활동에 mbox가 있는 경우 페이로드가 반환됩니다.
...
을(를) 지정한 후 `logging` = `client_side`에서 페이로드를 받습니다. `mbox` 아래 표시된 필드.
...
Target의 응답에 `analytics` -> `payload` 속성을 그대로 Adobe Analytics에 전달합니다. Analytics는 이 페이로드를 처리하는 방법을 알고 있습니다. 이 작업은 다음 형식을 사용하여 GET 요청에서 수행할 수 있습니다.
