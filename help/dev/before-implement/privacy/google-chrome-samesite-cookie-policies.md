...
description: ' [!DNL Adobe Target] 이 Google Chrome 버전 80과 함께 도입된 SameSite IETF 표준을 처리하는 방법과 이러한 정책을 준수하기 위해 필요한 사항에 대해 살펴봅니다.'
...
Google은 2020년 초에 출시되는 Chrome 80을 시작으로 사용자를 위한 새로운 쿠키 정책을 도입할 예정입니다. 이 문서에서는 새로운 SameSite 쿠키 정책에 대해 알아 두어야 할 모든 사항과 방법 [!DNL Adobe Target] 는 이러한 정책 및 을 사용할 수 있는 방법을 지원합니다 [!DNL Target] Google Chrome의 새로운 SameSite 쿠키 정책을 준수하기 위해
...
![SameSite 대화 상자](../assets/samesite.png)
...
## [!DNL Target]Google의 보안 모범 사례를 따르는 
...
&quot;SameSite가 없는 쿠키를 보호해야 함&quot; 옵션의 경우 Target의 도메인 간 추적 기능을 옵트인하지 않으면 의 자사 쿠키를 [!DNL Target] 은(는) 계속 작동합니다.
...
| at.js 1.*x* (도메인 간 추적 활성화됨) | 별도로 미치는 영향이 없습니다. | 귀하의 사이트에 대해 HTTPS 프로토콜을 활성화해야 합니다.<br />Target은 서드파티 쿠키를 사용하여 사용자를 추적하므로 Google에는 서드파티 쿠키가 `SameSite = None` 및 보안 플래그. “보호” 플래그를 사용하려면 귀하의 사이트가 HTTPS 프로토콜을 사용해야 합니다. |
...
업계에서 소비자를 위해 보다 안전한 웹을 만들기 위해 노력함에 따라 Adobe은 고객이 최종 사용자의 보안 및 개인정보 보호를 보장하는 방식으로 개인화된 경험을 제공할 수 있도록 지원하기 위해 노력을 아끼지 않고 있습니다. 이제 필요한 사항은 앞서 언급된 모범 사례를 따르고 을 활용하는 것입니다 [!DNL Target] Google Chrome의 새로운 SameSite 쿠키 정책을 준수하기 위해
