...
Adobe Experience Cloud는 데이터 제어자가 다음 작업을 완료할 수 있도록 하는 GDPR 지원 API를 제공합니다.
...
Target을 포함한 Experience Cloud 솔루션 전체의 모든 GDPR 및 CCPA 요청은 현재 GDPR API라고 하는 중앙 Adobe API를 통해 전달됩니다. 그런 다음 API는 데이터 관리자의 Experience Cloud 솔루션 세트에 걸친 요청을 완료합니다.
...
데이터 액세스 요청에 대한 응답에는 해당 방문자에 대한 Target 프로필의 요약이 포함되어 있습니다. 이 반환은 Experience Cloud GDPR API로 전송되고 데이터 관리자에게 응답이 전송됩니다.
